...
Filter deconvolved samples on theta plausibility, then run limma-trend.
...
is an optimiser signature, not a tissue composition. Seven normals have no theta
at all (all-NaN rows).
...
collapsing states, and flooring theta each iteration so components cannot be
absorbed).

What the filter does and does not buy
-------------------------------------
Keeping samples where the compartment of interest has usable theta removes the
worst artefacts. It does not certify the survivors: a sample at acinar 0.986 may
be genuinely acinar-rich or may have been pushed to that vertex. Nothing in theta
distinguishes those.

Empirically, in this run, profile quality degrades *gradually* with theta rather
than falling off a cliff -- correlation to the reference profile ran 0.760 at
theta=0.005 up to 0.851 at theta=0.581, monotone. Low-theta samples are noisier
(psi = Z/theta amplifies assignment noise by 1/theta), not prior-collapsed. Noise
...
def theta_report(theta: pd.DataFrame, samples: Sequence[str],
    ...
    """
    states = list(states) if states is not None else list(theta.columns)
    sub = theta.reindex(list(samples))
...
def drop_dead_columns(df: pd.DataFrame, label: str = "") -> pd.DataFrame:
    ...
    ``state_expression`` divides Z by theta and emits NaN where theta collapsed,
...
def filter_samples_by_theta(
    ...
    *,
    ...
    """Keep samples whose theta is plausible for ``compartment``.

    Parameters
    ----------
    min_theta
        Floor on the compartment of interest. Below this, psi = Z/theta amplifies
    ...
    _apply(~sub.isna().all(axis=1), "no theta (all-NaN row)")
    if compartment not in sub.columns:
        raise KeyError(f"{compartment!r} not in theta columns: {list(sub.columns)}")
    ...
    if verbose:
        ...
        for why, n in reasons.items():
            print(f"    -{n:3d}  {why}")
        if len(kept):
            print(f"    theta[{compartment}] range: "
...
def run_lfc(
    ...
    **limma_kw,
    ...
    """Filter both arms on theta, then run limma-trend.
    ...
    keep_t = filter_samples_by_theta(
        theta, psi_tumor.columns, compartment=compartment,
        min_theta=min_theta_tumor)
    print(" normal arm:")
    keep_n = filter_samples_by_theta(
        theta, psi_normal.columns, compartment=compartment,
        min_theta=min_theta_normal,
    ...
    T = drop_dead_columns(psi_tumor[keep_t], "tumour ")
    N = drop_dead_columns(psi_normal[keep_n], "normal ")
    ...
    th_n = theta.loc[N.columns, compartment]
    spread = float(th_n.max() / max(th_n.min(), 1e-12))
    if spread > 20:
        print(f"  theta spans {spread:.0f}x across normals -- "
