...
class GDC(object):
    ...
    def clean_gdc_files(self):
        ...
        self.df_stage, self.df_sample, self.df_files = (
            pd.DataFrame(),
            pd.DataFrame(),
            pd.DataFrame(),
        )
        ...
        self.SITE_MAP = {
            ...
            "TCGA-BRCA": {"ductal": "breast_ductal", "lobular": "breast_lobular"},
            "TCGA-LUAD": {"adenocarcinoma_generic": "lung_adenocarcinoma"},
            "TCGA-LUSC": {"squamous": "lung_squamous"},
        }
    ...
    def get_primary_sites(
        self, program: str = "TCGA", force: bool = False, verbose: bool = False
    ) -> pd.DataFrame:
        ...
        filters = {"op": "in", "content": {"field": "program.name", "value": [program]}}
        ...
        try:
            ...
            for col in df_ps.columns:
                df_ps[col] = df_ps[col].apply(lambda x: ", ".join(x) if isinstance(x, list) else x)
    ...
    def get_cases_and_subtypes(
        ...
        def build_tcga_ontology(df):
            ...
            df["subtype_tissue"] = df.apply(
                lambda r: self.map_tissue_subtype(r["subtype_global"], r["pid"]), axis=1
            )
        ...
        # -------------------------- batch loop ---------------------------
        filters = {"op": "in", "content": {"field": "cases.project.project_id", "value": [pid]}}
    ...
    def groupby_state(self, df_cases: pd.DataFrame):
        ...
        df_subt = (
            df_cases.groupby(
                ["pid", "subtype_global", "tumor_class", "subtype_tissue", "sstage"], dropna=False
            )
            .size()
            .reset_index(name="n")
        )
    ...
    def get_samples(
        ...
        filters = {
            "op": "and",
            "content": [
                {"op": "in", "content": {"field": "cases.project.project_id", "value": [pid]}},
                {
                    "op": "in",
        ...
        try:
            ...
            if verbose:
                print(f"Found {len(df_sample)} samples for many cases.")
    ...
    def get_expression_files_given_samples(
        self,
        pid: str,
        subtype: str,
        stage: str,
        ...
        try:
            ...
            if verbose:
                print(f"Found {len(df_files)} files in selected samples.")
    ...
    def get_files(self, case_uuid: str, data_type: str = "Gene Expression Quantification") -> List:
        lista = self.get_expression_files_by_uuid(
            case_uuid, data_type="Gene Expression Quantification"
        )
        return lista

    def get_expression_files_by_uuid(
        self, case_uuid: str, data_type: str = "Gene Expression Quantification"
    ) -> List:
    ...
    def download_file(self, force: bool = False, verbose: bool = True) -> bool:
        ...
        if os.path.exists(self.gdc_ouptut_filename) and not force:
            if verbose:
                print(f"File already downloaded and cleaned: {self.gdc_filename}")
            return True
        ...
        try:
            ...
            if verbose:
                print(f"File saved at {self.gdc_filename}.")
    ...
    def clean_gdc_tsv(self, force: bool = False, verbose: bool = True) -> pd.DataFrame:
        ...
        if os.path.exists(self.gdc_ouptut_filename) and not force:
            ...
            if verbose:
                print(f"Reading table {df2.shape}: {self.gdc_ouptut_filename}")
            return df2
        ...
        try:
            ...
            if verbose:
                print(f"Read table {df2.shape}: {self.gdc_ouptut_filename}")
        except Exception as e:
    ...
    def get_cases_deprecated(
        ...
        params = {"filters": json.dumps(filters), "fields": "case_id", "size": size_}
