...
- [3. Analysis history and design decisions](#3-analysis-history-and-design-decisions)
  ...
  - [3.5 Filtering: low counts and zero-θ samples](#35-filtering-low-counts-zero-θ-samples-and-vertex-collapse)
...
### 3.5 Filtering: low counts, zero-θ samples and vertex collapse

"Do not transform" (§3.4) is not "do not filter". Deconvolved compartment matrices carry
two failure modes that must be removed before any per-compartment model is fitted.

#### Gene filter — low counts

Applied inside `compartment_matrix` via `min_counts`:

<!-- TODO: state the exact rule, e.g. counts ≥ MIN_COUNTS in ≥ X% of retained samples -->

```python
X_fib2 = mc.compartment_matrix(
    cmap["fibroblast"],
    min_share=MIN_SHARE,      # TODO: value
    min_counts=MIN_COUNTS,    # TODO: value
    batch=batch,
    samples=tumor_samples,
)
```

Because the filter is applied **per compartment**, the two compartments end with different
gene universes:

| Compartment | Genes retained |
|---|---:|
| `X_mal2` (malignant) | 8,187 |
| `X_fib2` (fibroblast) | 11,464 |

> ⚠️ **Unexplained direction.** The malignant compartment carries the larger θ share in
> most tumours, so it should pass a count-based threshold more easily, not less. The
> smaller malignant universe therefore needs an explanation — most likely a
> detection-across-samples criterion interacting with basal/classical heterogeneity.
> These two denominators set the FDR scale in §6, so the discrepancy is not cosmetic.

> ✅ **Filtering on mean expression / detection is independent of the test statistic under
> the null and does not inflate type-I error** (Bourgon *et al.* 2010, PNAS). Filtering on
> **variance**, or on any statistic correlated with the contrast, does. No variance-based
> gene ranking is applied upstream of the factorial fit.

#### Sample filter — zero and near-zero θ

BayesPrism returns an expression vector for every compartment in every sample, including
compartments with no posterior mass behind them. Such samples pass a library-size check —
the *bulk* library is fine — but their compartment estimate is unidentified and enters the
design as noise.

Diagnostics over the 131 tumour samples (`T-` prefix):

```python
tumor_samples = [s for s in mc.df_theta.index if s.startswith("T-")]
th_t = mc.df_theta.loc[tumor_samples]

(th_t == 0).sum(axis=1).value_counts().sort_index()   # exact zeros per sample
(th_t < 1e-10).sum(axis=1).value_counts().sort_index() # underflow per sample
th_t.max(axis=1).describe(percentiles=[.9, .95, .99])
```

**Exact-zero compartments per sample**

| zeros | 0 | 1 | 2 | 3 |
|---|---:|---:|---:|---:|
| samples | 48 | 49 | 33 | 1 |

**Compartments below 1e−10 per sample (float64 underflow)**

| underflow | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| samples | 4 | 16 | 33 | 29 | 23 | 14 | 10 | 2 |

**Maximum θ per sample** (n = 130): mean 0.621, median 0.596, sd 0.179, min 0.330,
p90 0.886, p95 0.930, p99 0.997, max 1.000.

Three observations follow.

1. **Underflow is the norm, not the exception.** Only 4 of 131 tumours have no compartment
   below 1e−10; 49 have four or more. Vertex collapse (θ → 1.000 for one compartment, all
   others at underflow) is the extreme tail of this distribution, not a separate class of
   four outliers. Most affected compartments are the low-reliability immune ones
   (T cell ρ = 0.136, endothelial ρ = 0.189, macrophage ρ = 0.197, B cell ρ = 0.216).

2. **These counts are cross-compartment and do not decide inclusion in any single matrix.**
   A sample with θ_Tcell = 0 and θ_Bcell = 0 contributes two zeros while retaining a fully
   usable fibroblast estimate. The filter relevant to §6 is the **marginal** θ share of the
   target compartment:

   ```python
   for c in ["fibroblast", "malignant"]:
       s = mc.df_theta.loc[tumor_samples, cmap[c]].sum(axis=1)
       print(c, (s < MIN_SHARE).sum())   # TODO: record counts
   ```

3. **One sample carries a missing θ row.** `value_counts()` sums to 131 for both zero and
   underflow tallies, but `describe()` reports count = 130 — `describe()` drops NaN. That
   sample is currently inside `tumor_samples` and is passed to `compartment_matrix`.

   ```python
   th_t[th_t.isna().any(axis=1)]   # TODO: identify and exclude
   ```
...
| Stage | n |
|---|---:|
| Tumour samples (`T-` prefix) | 131 |
| With complete θ row | 130 |
| Entering the factorial fit (`n_samples`) | 108 |

<!-- TODO: document the 130 → 108 step -->

> ⚠️ **Both `R_mal` and `R_fib` report n_samples = 108.** If sample inclusion depended on
> the target compartment's θ share, the two compartments would be expected to retain
> different sample sets, since θ_malignant and θ_fibroblast have different distributions.
> Identical n implies either a compartment-independent QC list, or that `n_samples` reports
> something other than the rows entering the design. This must be resolved before the
> off-diagonal test in §6.5, because a shared sample set and a shared gene intersection are
> both prerequisites for comparing the two nulls.
